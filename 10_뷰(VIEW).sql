...
--한국에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 금무국가명 조회
SELECT *
FROM VW_EMPLOYEE
WHERE NATIONAL_NAME = '일본';
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME,
          ...
          EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIR_DATE) AS "근무년수"
    FROM EMPLOYEE
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME,
          ...
          EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIR_DATE)
    FROM EMPLOYEE
    JOIN JOB USING(JOB_CODE);
...
/*
    ...
    4) WITE READ ONLE : 뷰에 대해서 조회만 가능하도록
*/
